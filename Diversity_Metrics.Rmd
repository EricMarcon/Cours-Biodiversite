...
InstallPackages(c("tidyverse", "entropart", "secret", "openssl"))
...
```{r}
Abundances %>% filter(Plot==6) -> AbundancesP6
...
Formatting the data as a dataframe, species in lines, plots in columns.

```{r}
Abundances %>% 
  spread(key = Plot, value = Abundance, fill=0) %>% 
  as.data.frame -> df
